...
//! (PARITY_SPEC §4.9 as-built law). Forces per body:
//!
...
//!
//! Pure and allocation-free: the panel owns the [`PhysBody`] buffer and calls
//! [`step`] only while the sim is hot (the sleep gate lives in the sim).
...
mod tests {
    ...
    /// Step `n` frames at the web's max clamp (33ms → 30fps worst case).
